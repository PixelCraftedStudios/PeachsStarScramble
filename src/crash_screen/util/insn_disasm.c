...
const InsnTemplate* get_insn(InsnData* insnSrc) {
    ...
    switch (insn.opcode) {
        ...
        case OPC_COP2:
        case OPC_COP3:
            enum InsnType subtype = insn.cop_subtype;
            checkInsn = insn_db_cop_lists[insn.cop_num][subtype]; // Use COPz lists.
            if (checkInsn == NULL) {
                return NULL;
            }
            switch (subtype) {
        ...
        case OPC_SPECIAL:
            ...
            opcode    = insn.func; // The last 6 bits.
            break;
        case OPC_REGIMM:
            checkInsn = insn_db_regi;
            opcode    = insn.regimm; // The 5 bits after the first 11.
            break;
        default:
            checkInsn = insn_db_standard;
            opcode    = insn.opcode; // The first 6 bits.
            break;
    }
...
s16 insn_check_for_branch_offset(InsnData insn) {
    ...
    if (info != NULL) {
        ...
        const InsnParam* list = insn_param_formats[fmt];
...
// break/trap instruction codes generated by gcc:
// -mdivide-breaks or -mdivide-traps
...
const char* comment_from_insn(InsnData insn, const InsnTemplate* info) {
    const u16 insnHi = insn.hi;
    const char* comment = NULL;

    if ((insnHi == 0x27BD) || (insnHi == 0x67BD)) { // [addiu/daddiu] $sp, $sp, 0xXXXX
        comment = "deallocate stack";
        if ((info != NULL) && (info->pseudoC == PSI_SUBI)) {
            comment += STRLEN("de"); // "allocate stack"
        }
    } else if ((insnHi == 0xFFBF) || (insnHi == 0xAFBF)) { // [sw/sd] $ra, 0xXXXX($sp)
        comment = "RA to stack";
    } else if ((insnHi == 0x8FBF) || (insnHi == 0xD7BF)) { // [lw/ld] $ra, 0xXXXX($sp)
        comment = "RA from stack";
    } else if ((insnHi == 0xAFBE) || (insnHi == 0xFFBE)) { // [sw/sd] $fp, 0xXXXX($sp)
        comment = "&frame to stack"; //! TODO: assume $s8 if $s7 is used
    } else if ((insnHi == 0x8FBE) || (insnHi == 0xD7BE)) { // [lw/ld] $fp, 0xXXXX($sp)
        comment = "&frame from stack"; //! TODO: assume $s8 if $s7 is used
    } else if (insn.raw == 0x03A0F025) { // or $fp, $sp, $r0
        comment = "&frame = &stack";
    } else if (insn.raw == 0x03C0E825) { // or $sp, $fp, $r0
        comment = "&stack = &frame";
    } else if (((insnHi & 0xFFFE0) == 0x27C0)) { //! TODO: addiu/daddiu xx, $fp
        comment = "deallocate frame"; //! TODO: Is this correct?
    } else if (((insn.raw & 0xFFE0FFFF) == 0x03A0E823) || ((insn.raw & 0xFFE0FFFF) == 0x03A0E82F)) {
        comment = ("deallocate stack" + STRLEN("de"));
    }
...
char* cs_insn_to_string(Address addr, InsnData insn, const char** fname, _Bool formatting) {
    ...
    if (info != NULL) {
        ...
        for (size_t cmdIndex = 0; cmdIndex < MIPS_NUM_PARAMS; cmdIndex++) {
            ...
            switch (cmd) {
                ...
                case MP_EXC20: // For SYSCALL and BREAK instructions.
                    ADD_COLOR(COLOR_RGBA32_LIGHT_GRAY);
                    if (info->opcode == OPS_BREAK) { //! TODO: version of this for teq also (mdivide-traps)
                        u16 eA = insn.codeA;
                        u16 eB = insn.codeB;
        ...
        const char* comment = comment_from_insn(insn, info);
        if (comment != NULL) {
            ADD_COLOR(COLOR_RGBA32_VSC_COMMENT);
            ADD_STR(" # %s", comment);
        }
...
#define PSC_FP      '#'
    { .c = PSC_FP,     .col = PSC_COL_VAR,  .a = "FC", }, // FP condition
#define PSC_RA      '$'
...
static _Bool modify_pseudo_c_assignment_str(const char** formatStr, InsnData insn, MIPS_C_Pseudocodes pseudoC) {
    ...
    if (insn.rt == insn.rs) {
        switch (pseudoC) {
            case PSI_ADDI: s = "t += i"; break;
            // 15
            case PSI_ANDI: s = "t &= i"; break;
            case PSI_ORI:  s = "t |= i"; break;
            case PSI_XORI: s = "t ^= i"; break;
            // 19
            case PSI_SUBI: s = "t -= i"; break;
            default: break;
        }
    } else if (insn.rd == insn.rs) {
        switch (pseudoC) {
            case PSI_ADD: s = "d += t"; break;
            case PSI_SUB: s = "d -= t"; break;
            case PSI_AND: s = "d &= t"; break;
            case PSI_OR:  s = "d |= t"; break;
            case PSI_XOR: s = "d ^= t"; break;
            default: break;
        }
    } else if (insn.rd == insn.rt) {
        switch (pseudoC) {
            case PSI_SLI:    s = "d <<= a"; break;
            case PSI_SRI:    s = "d >>= a"; break;
            case PSI_DSLI32: s = "d <<= A"; break;
            case PSI_DSRI32: s = "d >>= A"; break;
            case PSI_SLV:    s = "d <<= s"; break;
            case PSI_SRV:    s = "d >>= s"; break;
            default: break;
        }
    } else if (insn.fd == insn.fs) {
        switch (pseudoC) {
            case PSI_ADDF: s = "D += T"; break;
            case PSI_SUBF: s = "D -= T"; break;
            case PSI_MULF: s = "D *= T"; break;
            case PSI_DIVF: s = "D /= T"; break;
            default: break;
        }
    }
...
char* cs_insn_to_pseudo_c(InsnData insn, const char** comment) {
    ...
    MIPS_C_Pseudocodes pseudoC = info->pseudoC;
                ...
                case PSC_CACHE:
                    ...
                    if (cacheName != NULL) {
                        ...
                        if (opList != NULL) {
                            ...
                            if (opName != NULL) {
                                strp += sprintf(strp, "%s.%s", cacheName, opName);
                                break;
                            }
                        }
                    }
